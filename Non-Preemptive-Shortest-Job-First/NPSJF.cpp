...
int main()
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < numberOfProcesses; i++)
        {
            if (!processList[i].isCompleted)
            {
                isAllCompleted = false;
                if (processList[i].arrivalTime <= currentTime)
                {
                    if (processList[i].burstTime < currentShortestJobRemainingTime)
                    {
                        currentShortestJobRemainingTime = processList[i].burstTime;
        ...
        currentTime += processList[currentShortestJobIndex].burstTime;
        processList[currentShortestJobIndex].isCompleted = true;
